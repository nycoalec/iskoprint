...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    /* Quick modules bar */
    .quick-modules {
      display: flex; flex-wrap: wrap; gap: 8px; padding: 12px 18px; border-bottom: 1px dashed var(--line);
      background: #fffaf8;
    }
    .quick-modules a {
      text-decoration: none;
    ...
    .chip {
      ...
      border:1px solid #e7e2e2; background:#ffffff; color: var(--maroon); font-weight:600; font-size: 12px;
      transition: transform .15s ease, box-shadow .15s ease, background-color .15s ease, color .15s ease;
    }
    .chip i { color: var(--maroon); }
    .chip:hover { transform: translateY(-1px); box-shadow: 0 6px 14px rgba(117,13,13,.18); background:#fef2f2; }
    ...
    /* Live Preview Editor */
    .editor {
      padding: 14px 18px 22px; border-bottom: 1px dashed var(--line);
    }
  ...
  <script>
    ...
    function sendEmail(event) {
      ...
      formData.append('service_type', 'printer');
      ...
      .catch(error => {
        ...
        ticker.textContent = 'Error: ' + error.message;
        ticker.classList.remove('sending', 'sent');
        ticker.classList.add('error');
      });
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      function loadPreviewFromFiles(){
        ...
        if (file.type.startsWith('image/')) {
          ...
          // Load PDF via PDF.js and render first page
      ...
      function renderPdfPage(pageNum){
        ...
        pdfDoc.getPage(pageNum).then(page => {
          // Render at high quality then scale with CSS per controls
          const desiredWidth = 1200; // base render width for quality
      ...
      window.openPrintPreview = function openPrintPreview(){
        ...
        if (currentFileType && currentFileType.startsWith('image/')) {
          ...
          // Use current rendered canvas as image for accurate look
